...
st.title("🌍 Populasi, Urbanisasi, Fertilitas & Harapan Hidup — Peta Dunia + Time Series")
st.write(
    "Halaman ini menampilkan indikator **demografi global** seperti *Population, Urban Population, Fertility Rate & Life Expectancy* "
    "berdasarkan file CSV yang berada dalam folder `data/`."
)
...
year_select = st.slider(
    ...
    int(min(years_sorted)), int(max(years_sorted)), int(max(years_sorted))
)
