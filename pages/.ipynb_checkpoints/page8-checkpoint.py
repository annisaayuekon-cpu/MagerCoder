...
st.title("🎓 Pendidikan — Peta Dunia + Time Series + Analisis")
st.write(
    "Halaman ini menampilkan indikator **pendidikan global** seperti *School Enrollment* "
    "dan *Government Expenditure on Education* berdasarkan file CSV di folder `data/`, "
    "lengkap dengan analisis statistik dan interpretasi otomatis."
)
...
def guess_unit_education(indicator: str) -> str:
    if "School Enrollment" in indicator:
        return "Umumnya **persentase (%)** (misal gross/net enrollment rate). Nilai lebih tinggi → partisipasi sekolah lebih tinggi."
    if "Expenditure" in indicator or "% of GDP" in indicator:
        return "Umumnya **persentase (% dari PDB)** untuk belanja pendidikan pemerintah. Nilai lebih tinggi → prioritas fiskal pendidikan lebih besar."
    return "Unit tidak terdeteksi (cek metadata file)."
...
def interpret_education(indicator: str, country: str, metrics: dict, unit_hint: str, rank_info: dict) -> str:
    if not metrics:
    ...
    if "School Enrollment" in indicator:
        meaning = (
            "Indikator ini mencerminkan **tingkat partisipasi sekolah** (enrollment). "
            "Kenaikan biasanya mengindikasikan akses pendidikan yang membaik."
        )
        policy_note = (
            "Jika enrollment tinggi tetapi stagnan, fokus kebijakan bisa bergeser ke **kualitas** (learning outcomes), "
            "bukan hanya akses."
        )
    else:
        meaning = (
            "Indikator ini menggambarkan **komitmen fiskal** pemerintah untuk pendidikan (persentase dari PDB). "
            "Kenaikan dapat mengindikasikan prioritas anggaran yang lebih tinggi."
        )
        policy_note = (
            "Perlu dibaca bersama efektivitas belanja: belanja naik belum tentu hasil meningkat bila alokasi tidak efisien."
        )
...
**Catatan analitis:**
- {policy_note}
- Untuk analisis lebih kuat, bandingkan dengan indikator hasil (mis. skor belajar, completion rate, PISA), jika tersedia.
"""
    return md.strip()
...
unit_hint = guess_unit_education(indicator)
...
with tab_analysis:
    ...
    with tabs2[3]:
        ...
        if df_c.empty:
            ...
            st.markdown(interpret_education(indicator, focus_country, metrics, unit_hint, rank_info))
